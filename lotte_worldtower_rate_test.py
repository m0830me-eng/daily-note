...
def classify_event(row):
    ...
    if (
        accompany_code == "30"
        or "무대인사" in accompany_compact
    ):
        return "STAGE"

    if (
        accompany_code == "40"
        or "관객과의대화" in accompany_compact
        or bounded_gv(accompany_text)
    ):
        return "GV"

    # --------------------------------------------------------
    # 미래 롯데 필드 변경 대비 fallback
    # --------------------------------------------------------
            ...
            if start and screen:
                ...
                rows.append({
                    ...
                    "screen_id": norm(
                        value.get("ScreenID")
                        or value.get("ScreenId")
                        or value.get("ScreenCode")
                    ),

                    "play_sequence": norm(
                        value.get("PlaySequence")
                        or value.get("PlaySeq")
                        or value.get("Sequence")
                    ),

                    "remain": norm(
                        value.get("BookingSeatCount")
                        or value.get("RemainSeatCount")
                        or value.get("RemainingSeatCount")
                    ),

                    "total": norm(
                        value.get("TotalSeatCount")
                        or value.get("SeatCount")
                    ),
...
def scan_dates(
    ...
    # 전체 범위에서 단 한 행도 못 받았을 때만 fallback.
    if (
        not all_rows
...
def main():
    log("=" * 60)
    log("LOTTE CINEMA WORLDTOWER MONITOR")
    log("=" * 60)
    log("대상: GV / 무대인사")
