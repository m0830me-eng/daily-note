...
def lotte_post(payload):
    """
    롯데 서버의 순간적인 연결 종료/빈 응답/비정상 JSON을 자동 복구한다.

    재시도 대상
    - ConnectionError / Timeout
    - JSONDecodeError 계열(ValueError: 빈 본문/HTML/깨진 JSON 포함)
    - 일시적인 HTTP 429/500/502/503/504

    총 3회까지 시도한다.
    1차 실패 -> 새 세션 + 0.5초 후 재시도
    2차 실패 -> 새 세션 + 1.0초 후 재시도
    3차도 실패 -> 기존 코드처럼 예외를 위로 올려 해당 날짜만 오류 처리

    403 같은 차단 응답은 무작정 반복하지 않고 즉시 예외 처리한다.
    """
    files = {
        "paramList": (
            None,
...
def alert_line(show):
    ...
    # 긴 예매 URL은 본문에 노출하지 않고 회차 문구 자체를 링크로 만든다.
...
def send_alert_group(
    ...
    # 같은 날짜 + 같은 종류 + 같은 상태의 회차는 한 메시지로 묶는다.
    # Discord 2000자 제한에 걸릴 정도로 많으면 같은 머리말로 안전하게 분할한다.
    current = list(header)
...
def process(
    ...
    for key, show in events.items():
        ...
        if alert_status:
            # 바로 회차별로 전송하지 않는다.
            # 같은 스캔에서 잡힌 동일 날짜/종류 회차를 아래에서 한 번에 묶는다.
    ...
    for item in pending_alerts:
        key, show, alert_status, previous = item
        group_key = (
            alert_status,
            show.get("event_type"),
            show.get("date"),
    ...
    for group_key, items in grouped.items():
        ...
        try:
            ...
            # 전송 실패 시 상태를 갱신하지 않아 다음 사이클에서 다시 시도한다.
            continue

        for key, show, _, previous in items:
            stored_status = (
                "OPEN"
